...
<div class="content-text">
    ...
    {{ Form::open(['action' => 'Tahap1Controller@upload', 'files' => true]) }}
    <div class="form-group row">
        {{ Form::label('DownloadSoal', 'Download Soal', ['class' => 'col-form-label col-sm-3 col-xs-12']) }}
        <div class="col-sm-9 col-xs-12">
            <button type="button" id="DownloadSoal" class="btn btn-choose btn-download">Download</button>
        </div>
    </div>
    <div class="form-group row">
        {{ Form::label('DownloadLembarJawaban', 'Lembar Jawaban', ['class' => 'col-form-label col-sm-3 col-xs-12']) }}
        <div class="col-sm-9 col-xs-12">
            <button type="button" id="DownloadLembarJawaban" class="btn btn-choose btn-download">Download</button>
        </div>
    </div>
    <div class="form-group row">
...
<script>
    ...
    $(document).ready(function () {
        $.post('/timeline', { id : 3 } ,function (data) {
    ...
    function DownloadFile(button) {
        let type = button.prop('id').trim();
        var url = "";
        switch (type) {
            case "DownloadSoal":
                url = "{{ route('file-download', ['file' => Crypt::encrypt('SoalTahap1')]) }}";
                break;
        
            case "DownloadLembarJawaban":
                url = "{{ route('file-download', ['file' => Crypt::encrypt('LembarJawaban')]) }}"
                break;

            default:
                alert("Something wrong has happened. Restart the page!");
                break;
        }
